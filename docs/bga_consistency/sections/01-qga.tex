...
Isabelle/Pure.  The propositional and arithmetic core, and the definitional
mechanism are from Kehrli's development, who established
that grounded arithmetic can be axiomatized in Pure and reasoned in
productively.\footnote{Sascha Kehrli, \emph{Formalizing Grounded Arithmetic atop
Isabelle/Pure}, BSc thesis, Distributed Computing Group, ETH Z\"urich, 2025.}
...
The rules in the rest of this section are written without a hypothesis context,
because Pure maintains one.  Making it visible for a moment, a \QGA{} judgment is
$\Gamma \seq \varphi$ with $\Gamma$ a finite set of formulas, and the framework
supplies the operations that move judgments around.

\begin{rulesiii}
\iaxiom{\Gamma, \varphi \seq \varphi}{hyp} &
\irule{\Gamma \seq \varphi}{\Gamma, \psi \seq \varphi}{weak} &
\irule{\Gamma \seq \psi \sep \Gamma, \psi \seq \varphi}{\Gamma \seq \varphi}{cut}\\[3.2ex]
\multicolumn{3}{c}{\irule{\Gamma \seq \varphi}
  {[x \mapsto t]\Gamma \seq [x \mapsto t]\varphi}{inst}}
\end{rulesiii}

\noindent
None of these is an axiom of \texttt{GD.thy}.  \textsc{hyp} is Pure's
\isa{assume}, \textsc{weak} and \textsc{cut} are introduction and elimination of
\isa{⟹}, and \textsc{inst} is instantiation of a schematic variable.  A rule
written \isa{⟦P; Q⟧ ⟹ R} therefore means the judgment $P, Q \seq R$, and a
theorem written that way holds for every instance of its free variables.
Contraction and exchange do not appear at all, since $\Gamma$ is a set.

One further operation has no sequent form.  If $\Gamma \seq \varphi$ is derived
for a variable $x$ that occurs nowhere in $\Gamma$, the derivation is uniform in
$x$, and Pure records this as \isa{⋀x}.  This is the eigenvariable condition, and
it is what the premises of \textsc{Ind} and $\forall$I below rely on.
...
eigenvariable, $\forall$E instantiates only at terminating terms, and $\exists$I
must exhibit a terminating witness.

Under these four rules a negated universal is inert.  \isa{forall} is a
primitive constant, so no elimination rule matches $\neg(\forall x.\,Q\,x)$: the
De Morgan rules $\vee$E2 and $\vee$E3 want a negated disjunction, $\neg\neg$E
wants a double negation, and \textsc{exF} would first need $\forall x.\,Q\,x$
itself, which $\forall$I supplies only from a uniform derivation.  Nor does any
rule conclude $\neg(\forall x.\,Q\,x)$ from a counterexample.  Two further
axioms supply both directions.

\begin{rules}
\irule{a \N \sep \neg (Q\,a)}{\neg (\forall x.\,Q\,x)}{$\neg\forall$I} &
\irule{\neg (\forall x.\,Q\,x) \sep \dis{a \N,\; \neg (Q\,a)}{R}}{R}{$\neg\forall$E}
\end{rules}

\thy{gd-notforall}

\noindent
$\neg\forall$I refutes a universal from one terminating counterexample and
$\neg\forall$E extracts one, so together they restore the duality between
$\neg\forall$ and $\exists\neg$ as a pair of inference rules.  Each carries a
habeas quid premise on the witness, which keeps them in line with the rest of the
system: the counterexample has to be an actual number rather than a description
of one.  \S\ref{sec:semantics:formulas} gives the semantic clause that justifies
them.  They correspond to the rules taken as primitive in reflective grounded
arithmetic, which are primitive there for the same reason.

The dual pair for the existential needs no axiom.  From $\neg(\exists
x.\,Q\,x)$, together with groundedness of $Q$ at the point under consideration,
$\exists$I manufactures the positive partner and \textsc{exF} closes the case.
This gives
\begin{center}
\isa{existsNeg: ⟦¬(∃x. Q x); ⋀x. x N ⟹ (Q x) B⟧ ⟹ ∀x. ¬(Q x)}
\end{center}
as a derived rule.  The asymmetry is the one just described.  $\exists$ has an
introduction rule that applies to a local assumption, and $\forall$ does not.

Where the new axioms are used is worth recording, since it is a single place.
\S\ref{sec:proof} needs to decide whether a list of hypotheses is satisfied, and
the negative case of that decision is the refutation of a universal.  Nothing
else in the development appeals to either rule.
...
\multicolumn{3}{@{}l}{\textbf{Structural (supplied by the framework)}}\\[0.9ex]
\iaxiom{\Gamma, \varphi \seq \varphi}{hyp} &
\irule{\Gamma \seq \varphi}{\Gamma, \psi \seq \varphi}{weak} &
\irule{\Gamma \seq \psi \sep \Gamma, \psi \seq \varphi}{\Gamma \seq \varphi}{cut}\\[2.4ex]
\multicolumn{3}{c}{\irule{\Gamma \seq \varphi}
  {[x \mapsto t]\Gamma \seq [x \mapsto t]\varphi}{inst}}\\[2.8ex]
...
\multicolumn{3}{@{}l}{\textbf{Quantifiers}}\\[0.9ex]
\irule{\dis{x \N}{Q\,x}}{\forall x.\,Q\,x}{$\forall$I} &
\irule{\forall x.\,Q\,x \sep a \N}{Q\,a}{$\forall$E} &
\irule{a \N \sep Q\,a}{\exists x.\,Q\,x}{$\exists$I}\\[3.6ex]
\irule{\exists x.\,Q\,x \sep \dis{a \N,\; Q\,a}{R}}{R}{$\exists$E} &
\irule{a \N \sep \neg (Q\,a)}{\neg (\forall x.\,Q\,x)}{$\neg\forall$I} &
\irule{\neg (\forall x.\,Q\,x) \sep \dis{a \N,\; \neg (Q\,a)}{R}}{R}{$\neg\forall$E}\\[2.8ex]
