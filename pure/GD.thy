...
let
  val {simps, congs, simprocs, ...} = Raw_Simplifier.dest_ss ss
  fun name_sthm ((nm,_), thm) =
  ...
  Pretty.big_list "Simproc patterns:" (map name_trm simprocs)]
in
pps |> Pretty.chunks
    |> pwriteln
end
...
ML \<open>
let
  val ss1 = put_simpset empty_ss @{context} addsimps
  [@{thm predSucInv} RS @{thm eq_reflection}]
  val ss2 = ss1 |> Simplifier.add_cong (@{thm sucCong} RS @{thm eq_reflection})
in
...
structure GD_Simp =
struct
  fun reflect_eq th = th RS @{thm eq_reflection};
...
lemmas [simp] = condI2 condI1 condI3 predSucInv neq_def
...
lemma less_0_false [simp, gd_auto]: "(x < 0) = 0"
...
lemma mult_one [simp, gd_auto]:
  shows "a N \<Longrightarrow> a * 1 = a"
by (unfold_def def_mult, simp, fold isNat_def)

lemma plus_one_suc [simp, gd_auto]:
...
lemma [gd_auto]: "x N \<Longrightarrow> x = x"
by (fold isNat_def)

lemma one_mult [simp, gd_auto]:
...
lemma zero_leq_true [simp, gd_auto]:
  assumes x_nat: "x N"
  shows "0 \<le> x = 1"
...
lemma [gd_auto]: "x N \<Longrightarrow> \<not> 0 = S x"
apply (fold neq_def)
apply (rule neq_sym)
apply (simp)
done
...
lemma leq_monotone_suc [simp, gd_auto]:
...
lemma suc_pred_inv [simp, gd_auto]:
          ...
          show "\<forall>xa y. xa \<le> y = 1 \<turnstile> y \<le> S x = 1 \<turnstile> xa \<le> S x = 1"
            proof (rule forallI entailsI)+
              ...
              show "xa \<le> S x = 1"
                ...
                apply (rule xa_nat)
                apply (rule x_nat)+
                apply (subst predSucInv)
                apply (rule x_nat)
                apply (rule eqSubst[where a="x" and b="P(S(x))"])
                apply (rule eqSym)
                apply (gd_auto)
                apply (rule x_nat)
                apply (rule disjE1[where P="xa=0" and Q="\<not>(xa=0)"])
                apply (fold GD.bJudg_def)
                apply (rule eqBool)
                apply (rule xa_nat)
                apply (rule nat0)
                ...
                proof -
                  ...
                  have H3: "P(xa) \<le> P(ya) = 1"
                    apply (rule leq_monotone_pred)
                    apply (rule xa_nat)
                    apply (rule ya_nat)
                    apply (rule xa_leq_ya)
                    done
                  have H4: "P xa \<le> P ya = 1 \<turnstile> P ya \<le> x = 1 \<turnstile> P xa \<le> x = 1"
                  ...
                  then have H5: "P ya \<le> x = 1 \<turnstile> P xa \<le> x = 1"
                    apply (rule entailsE)
                    apply (rule H3)
                    done
                  then show "P xa \<le> x = 1"
                    apply (rule entailsE)
                    apply (rule H2)
                    done
                qed
...
lemma zero_less_true [simp, gd_auto]:
  assumes a_nat: "a N"
  shows "0 < S(a) = 1"
apply (unfold_def def_less)
apply (rule condI2Eq)
...
lemma sub_0 [simp, gd_auto]:
  shows "x N \<Longrightarrow> x - 0 = x"
by (unfold_def def_sub, simp)

lemma zero_div [simp, gd_auto]:
  assumes x_nat: "x N"
  shows "x N \<Longrightarrow> div 0 S(x) = 0"
by (unfold_def def_div, simp)

lemma div_1 [simp, gd_auto]:
  assumes x_nat: "x N"
...
proof (rule GD.grounded_contradiction[where q="x = y"])
  show "\<not> P x = P y B" by (gd_auto, rule x_nat, rule y_nat)
...
lemma le_suc_implies_leq:
  ...
  shows "x \<le> y = 1"
proof -
  have "\<forall>x. x < S y = 1 \<turnstile> x \<le> y = 1"
  proof (rule ind[where a="y"])
    show "y N" by (rule y_nat)
    ...
    show "\<forall>x. (\<forall>xa. xa < S x = 1 \<turnstile> xa \<le> x = 1) \<turnstile>
     (\<forall>xa. xa < S S x = 1 \<turnstile> xa \<le> S x = 1)"
      ...
      qed
  qed
  then have "x < S y = 1 \<turnstile> x \<le> y = 1"
    apply (rule forallE)
    apply (rule x_nat)
    done
  then show ?thesis
...
proof (rule contradiction)
  ...
  show "\<not> x = S(y) \<Longrightarrow> False"
    proof -
      ...
      have H4: "x \<le> y = 1" by (rule le_suc_implies_leq, rule x_nat, rule y_nat, rule H)
      show "False"
        apply (rule exF[where P="x \<le> y = 1"])
        apply (rule H4)
        apply (rule x_ge_y)
...
lemma leq_monotone [gd_auto, simp]:
  assumes x_nat: "x N"
  assumes y_nat: "y N"
  shows "x \<le> y + x = 1"
proof (rule ind[where a="x"])
...
apply (rule ind[where a="x"])
apply (rule x_nat)
apply (rule eqSubst[where a="S y" and b="S y + 0"])
apply (rule eqSym)
apply (gd_auto)
apply (rule y_nat)
apply (unfold_def def_add)
...
lemma cpair_mono_r:
  assumes x_nat: "x N"
  assumes y_nat: "y N"
  shows "y \<le> \<langle>x, y\<rangle> = 1"
apply (unfold cpair_def)
apply (rule add_leq_monotone)
apply (rule y_nat)
apply (rule y_nat)
apply (gd_auto)
apply (rule x_nat)
...
lemma neq_monotone_add:
  ...
  shows "\<not> y = x + y"
proof -
  have H1: "\<And>xa. xa N \<Longrightarrow> \<not> (y = S(xa) + y)"
    proof -
      fix xa
      assume xa_nat: "xa N"
      show "\<not> (y = S(xa) + y)"
        ...
        qed
    qed
  have "\<exists>p. x = S(p)" by (gd_auto, rule x_nat, rule x_nz)
  then show ?thesis
    apply (rule existsE)
    proof -
      fix a
...
lemma swap_add:
  ...
  assumes H: "Q (y + x)"
  shows "Q (x + y)"
apply (rule eqSubst[where a="y+x" and b="x+y"])
apply (gd_auto)
apply (rule y_nat)
apply (rule x_nat)
apply (rule H)
...
proof -
  ...
  have x_neq_y: "\<not> x - y = 0"
    apply (rule nz_monotone_leq[where x="z"])
    apply (rule z_nat)
    apply (gd_auto)
    apply (rule x_nat)
    apply (rule y_nat)
    apply (rule eqSubst[where a="P(x-y)" and b="S(z)"])
  ...
  then have H1: "S(P(x - y)) = S(S(z))"
    by (rule sucCong)
  show "x - y = S(S(z))"
    apply (rule eqSubst[where b="x - y" and a="S P (x - y)"])
    apply (rule suc_pred_inv)
    apply (gd_auto)
    apply (rule x_nat)
    apply (rule y_nat)
  ...
  have "x - y = S(z) \<turnstile> x - S(y) = z"
    ...
    proof (rule forallI entailsI)+
      ...
      show "x - S(S xa) = z"
        ...
        apply (gd_auto)
        apply (rule xa_nat)
        apply (rule eqSubst[where a="S z" and b="x - S xa"])
        apply (rule eqSym)
        apply (rule h)
        apply (rule replace_ps)
        apply (rule z_nat)
...
lemma sub_eq_0_imp_leq:
  ...
  shows "a \<le> b = 1"
proof -
  have "\<forall>a. a - b = 0 \<turnstile> a \<le> b = 1"
    ...
    show "\<forall>x. (\<forall>a. a - x = 0 \<turnstile> a \<le> x = 1) \<turnstile>
     (\<forall>a. a - S x = 0 \<turnstile> a \<le> S x = 1)"
      proof (rule forallI, rule entailsI, rule forallI, rule entailsI)
        fix xa aa
        assume xa_nat: "xa N" and aa_nat: "aa N"
        ...
        show "aa \<le> S xa = 1"
          ...
          apply (assumption)
          proof -
            show "aa N" by (rule aa_nat)
            show "aa - xa N" by (simp, rule aa_nat, rule xa_nat)
            fix a
            assume aa_sub_xa_nz: "\<not> aa - xa = 0" and aa_nz: "\<not> aa = 0" and a_nat: "a N" and aa_eq_sa: "aa = S a"
            show "aa \<le> S xa = 1"
              ...
              apply (rule aa_eq_sa)
              apply (rule eqSubst[where a="S P (aa - xa)" and b="aa - xa"])
              apply (gd_auto)
              apply (rule aa_nat)
              apply (rule xa_nat)
      ...
      qed
  qed
  then have "a - b = 0 \<turnstile> a \<le> b = 1"
    apply (rule forallE)
    apply (rule a_nat)
    done
  then show ?thesis
              ...
              proof (rule forallI entailsI)+
                ...
                show "xa < S x = 1"
                  ...
                  proof -
                    ...
                    have H4: "P xa < P ya = 1 \<turnstile> P ya < x = 1 \<turnstile> P xa < x = 1"
                      apply (rule forallE[where a="P ya"])
                      apply (rule forallE[where a="P xa"])
                      apply (rule hyp)
                      apply (gd_auto)
                    ...
                    then have H5: "P ya < x = 1 \<turnstile> P xa < x = 1"
                      apply (rule entailsE)
                      apply (rule H3)
                      done
                    then show "P xa < x = 1"
...
proof (rule forallI entailsI)+
  ...
  show "S x - S S xa < S x = 1"
    ...
    apply (rule x_nat)
    apply (rule xa_nat)
    apply (rule cases_nat[where x="S(x) - S(xa)"])
    apply (rule eqSubst[where a="0" and b="P(0)"])
    apply (rule eqSym)
    apply (gd_auto)
    apply (rule x_nat)
    ...
    apply (gd_auto)
    apply (rule x_nat)
    apply (rule eqSubst[where a="S x - S xa" and b="S P (S x - S xa)"])
    apply (rule eqSym)
    apply (gd_auto)
    apply (rule x_nat)
    apply (rule xa_nat)
    apply (assumption)
    apply (rule less_trans[where y="S x"])
    apply (gd_auto)
    apply (rule x_nat)
    apply (rule xa_nat)
    apply (gd_auto)
...
lemma "a N \<Longrightarrow> a - a = 0"
apply (unfold_def def_sub)
sorry

lemma div_x_x_1 [gd_auto]:
  shows "x N \<Longrightarrow> div (S x) (S x) = 1"
proof (rule ind[where a="x"], gd_auto, rule forallI, rule entailsI)
  fix xa
  assume hyp: "div (S xa) (S xa) = 1"
  show "xa N \<Longrightarrow> div (S S xa) (S S xa) = 1"
    apply (unfold_def def_div)
    apply (subst less_not_refl)
    apply (rule condI2Eq)
    apply (fold neq_def)
    apply (gd_auto)
    sorry
...
lemma cpair_1_0_1 [simp, gd_auto]: "\<langle>1, 0\<rangle> = 1"
unfolding cpair_def
apply (subst add_zero)
apply (subst zero_add)
apply (subst one_mult)
done
...
proof (rule grounded_contradiction[where q="S(y) < S(y) = 1"])
  show "x = 0 B" by (gd_auto, rule x_nat)
  assume x_nz: "\<not> x = 0"
  have H1: "S(y) - x < S(y) = 1"
    apply (rule existsE[where Q="\<lambda>c. x = S(c)"])
    apply (gd_auto)
...
lemma cpair_strict_mono_r:
  ...
  assumes y_nat: "y N"
  assumes y_nz: "\<not> y = 0"
  shows "y < \<langle>x, y\<rangle> = 1"
apply (rule less_is_leq_neq)
apply (rule y_nat)
apply (gd_auto)
apply (rule x_nat)
apply (rule y_nat)
apply (rule cpair_mono_r)
apply (rule x_nat)
apply (rule y_nat)
apply (unfold cpair_def)
apply (rule swap_add)
...
where
  ...
  list_injective: "(Cons a x) = (Cons b y) \<Longrightarrow> a = b \<and> (x = y)" and
  ...
  list_decode: "list_decode := (\<lambda>x.
                             if (x = \<langle>0,0\<rangle>) then Nil
                             else if (x = \<langle>0,1,n,z'\<rangle>) then Cons n (list_decode z')
                             else omega)"

lemma zero_is_cons: "list_encode Nil = 0"
...
lemma list_encode_nil: "list_encode Nil = \<langle>0, 0\<rangle>"
apply (unfold_def list_encode)
apply (rule condI1)
apply (rule refl)
done

lemma list_encode_cons:  "list_encode (Cons n xs) = \<langle>0, 1, n, list_encode xs\<rangle>"
apply (unfold_def list_encode)
apply (rule condI2Eq)
...
apply (gd_auto)
apply (gd_auto)
apply (rule less_trans[where y="\<langle>n, xs\<rangle>"])
apply (gd_auto)
apply (gd_auto)
